...
class Version
{
	...
	{
		if (strpos(Forum::version(), 'GIT') !== false)
		{
		...
		{
			if (strpos(Forum::version(), 'DEV') !== false)
			{
			...
			{
				if (strpos(Forum::version(), 'RC') !== false)
				{
				...
				{
					if (strpos(Forum::version(), 'BETA') !== false)
					{
					...
					{
						if (strpos(Forum::version(), 'ALPHA') !== false)
						{
		...
		{
			return Text::sprintf($msg, '<strong>' . strtoupper(Forum::version()), $kn_version_type . '</strong>') . ' ' . $kn_version_warning;
		}
	...
	public function getDBVersion($prefix = 'kunena_')
	{
		...
		{
			if ($version->version != Forum::version())
			{
				$version->state = '';
			}
		}
	...
	{
		return 'Kunena ' . strtoupper(Forum::version()) . ' | ' . Forum::versionDate() . ' [ ' . Forum::versionName() . ' ]';
	}
