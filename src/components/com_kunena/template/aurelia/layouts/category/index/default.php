...
foreach ($this->sections as $section) :
	...
	$Itemid = \Joomla\Component\Kunena\Libraries\Route\KunenaRoute::getCategoryItemid($section);
	...
	<div class="kfrontend shadow-lg rounded border section">
		...
		<h1 class="card-header">
			<?php echo $this->getCategoryLink($section, $this->escape($section->name), null, \Joomla\Component\Kunena\Libraries\Template\Template::getInstance()->tooltips(), true, false); ?>
		...
		<div class="<?php if (!empty($section->class)) : ?>section<?php echo $this->escape($section->class_sfx); ?><?php endif; ?> collapse show shadow-lg rounded"
			...
			<table class="table<?php echo \Joomla\Component\Kunena\Libraries\Template\Template::getInstance()->borderless(); ?> table-responsive w-100 d-block d-md-table">
					...
					foreach ($this->categories[$section->id] as $category) : ?>
						<tr class="category<?php echo $this->escape($category->class_sfx); ?>"
							...
							<td colspan="6">
								<div>
									<h3>
										<?php echo $this->getCategoryLink($category, $category->name, null, \Joomla\Component\Kunena\Libraries\Template\Template::getInstance()->tooltips(), true, false); ?>
										<small class="nowrap">
											...
											<span>
												...
												<?php if (\Joomla\Component\Kunena\Libraries\KunenaFactory::getConfig()->enablerss) : ?>
									...
									<div>
										<ul class="list-inline">
											...
											<?php foreach ($this->categories[$category->id] as $subcategory) : ?>
												<li>
													...
													<?php if (Config::getInstance()->showchildcaticon) : ?>
														<?php echo $this->getCategoryLink($subcategory, $this->getSmallCategoryIcon($subcategory), '', null, true, false) . $this->getCategoryLink($subcategory, '', null, \Joomla\Component\Kunena\Libraries\Template\Template::getInstance()->tooltips(), true, false) . '<small class="hidden-xs-down muted"> ('
															. $totaltopics . ')</small>';
													else : ?>
														<?php echo $this->getCategoryLink($subcategory, '', null, \Joomla\Component\Kunena\Libraries\Template\Template::getInstance()->tooltips(), true, false) . '<small class="hidden-xs-down muted"> ('
															. $totaltopics . ')</small>';
													endif;
											...
											<?php if (!empty($this->more[$category->id])) : ?>
												<li>
													<?php echo $this->getCategoryLink($category, Text::_('COM_KUNENA_SEE_MORE'), null, \Joomla\Component\Kunena\Libraries\Template\Template::getInstance()->tooltips(), true, false); ?>
								...
								<?php if ($category->getmoderators() && Config::getInstance()->listcat_show_moderators) : ?>
									...
									<div class="moderators">
										...
										{
											$modslist[] = \Joomla\Component\Kunena\Libraries\KunenaFactory::getUser($moderator)->getLink(null, null, '', null, \Joomla\Component\Kunena\Libraries\Template\Template::getInstance()->tooltips());
										}
								...
								<?php if (!empty($this->pending[$category->id])) : ?>
									<div class="alert alert-warning" role="alert" style="margin-top:10px;">
										<a class="alert-link"
										   href="<?php echo \Joomla\Component\Kunena\Libraries\Route\KunenaRoute::_('index.php?option=com_kunena&view=topics&layout=posts&mode=unapproved&userid=0&catid=' . intval($category->id)); ?>"
								...
								<td colspan="5" class="hidden-xs-down">
									<div class="row">
										...
										<div class="col-xs-6 col-md-3" id="kcat-avatar">
											<?php echo $author->getLink($avatar, null, '', '', \Joomla\Component\Kunena\Libraries\Template\Template::getInstance()->tooltips(), $category->id, Config::getInstance()->avataredit); ?>
										</div>
										<div class="col-xs-6 col-md-9" id="kcat-last">
											<?php else : ?>
											<div class="col-md-12" id="kcat-last">
												<?php endif; ?>
												<span class="lastpostlink"><?php echo $this->getLastPostLink($category, null, null, \Joomla\Component\Kunena\Libraries\Template\Template::getInstance()->tooltips(), 30, false, true) ?></span>
												<br>
												<span class="lastpostby"><?php echo Text::sprintf('COM_KUNENA_BY_X', $author->getLink(null, null, '', '', \Joomla\Component\Kunena\Libraries\Template\Template::getInstance()->tooltips(), $category->id)); ?></span>
												<br>
